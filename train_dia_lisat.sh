...
DATASET="${DATASET:-sem_seg||refer_seg||correct_refer_seg||vqa||neg_refer_seg||reason_seg||geo_reason_seg||dia_align_aug}"
SAMPLE_RATES="${SAMPLE_RATES:-15,15,2,25,1,1,31,10}"
...
DIA_ALIGN_AUG_SOURCE_DATASET="${DIA_ALIGN_AUG_SOURCE_DATASET:-sem_seg||refer_seg||reason_seg||geo_reason_seg}"
DIA_ALIGN_AUG_SOURCE_RATES="${DIA_ALIGN_AUG_SOURCE_RATES:-15,15,1,36}"
DIA_ALIGN_AUG_POSITIVE_PROB="${DIA_ALIGN_AUG_POSITIVE_PROB:-0.5}"
DIA_ALIGN_AUG_BACKGROUND_SCALE="${DIA_ALIGN_AUG_BACKGROUND_SCALE:-0.2}"
DIA_ALIGN_AUG_MASK_DILATION="${DIA_ALIGN_AUG_MASK_DILATION:-2}"
...
DIA_NUM_EVIDENCE_TOKENS="${DIA_NUM_EVIDENCE_TOKENS:-4}"
...
PRESENCE_LOSS_WEIGHT="${PRESENCE_LOSS_WEIGHT:-0.20}"
PRESENCE_HIDDEN_DIM="${PRESENCE_HIDDEN_DIM:-256}"
PROMPT_DISTILL_LOSS_WEIGHT="${PROMPT_DISTILL_LOSS_WEIGHT:-0.10}"
PROMPT_DISTILL_DECAY_STEPS="${PROMPT_DISTILL_DECAY_STEPS:-4000}"
...
DIA_FUSION_MODE="${DIA_FUSION_MODE:-decoupled_evidence_prompt}"
...
ARGS=(
  ...
  --dia_align_aug_source_dataset "$DIA_ALIGN_AUG_SOURCE_DATASET"
  --dia_align_aug_source_rates "$DIA_ALIGN_AUG_SOURCE_RATES"
  --dia_align_aug_positive_prob "$DIA_ALIGN_AUG_POSITIVE_PROB"
  --dia_align_aug_background_scale "$DIA_ALIGN_AUG_BACKGROUND_SCALE"
  --dia_align_aug_mask_dilation "$DIA_ALIGN_AUG_MASK_DILATION"
...
if [ "${USE_DIA}" = "1" ]; then
  ARGS+=(
    ...
    --presence_hidden_dim "$PRESENCE_HIDDEN_DIM"
    --prompt_distill_loss_weight "$PROMPT_DISTILL_LOSS_WEIGHT"
    --prompt_distill_decay_steps "$PROMPT_DISTILL_DECAY_STEPS"
...
if [[ "${DATASET}" == *"dia_align_aug"* ]]; then
  echo "  DIA align aug: source=${DIA_ALIGN_AUG_SOURCE_DATASET}, source_rates=${DIA_ALIGN_AUG_SOURCE_RATES}, positive_prob=${DIA_ALIGN_AUG_POSITIVE_PROB}, background_scale=${DIA_ALIGN_AUG_BACKGROUND_SCALE}, mask_dilation=${DIA_ALIGN_AUG_MASK_DILATION}"
fi
