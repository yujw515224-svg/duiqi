...
#
# Everything outside the "DIA" block is the plain LISAt stage-2 recipe. The DIA
# flags all have defaults, so that block only exists to make the choices
...
# Mixing in the natural-image sets the LISAt paper also used (needs the RefCOCO /
# ReasonSeg data and scikit-image installed):
#   --dataset="geo_reason_seg||reason_seg||refer_seg" --sample_rates="12,1,3"
#
# Ablations (docs/DIA_README.md section 4):
